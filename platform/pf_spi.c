...
void SPIClearBuf(unsigned int moduleId,unsigned char cs){
    unsigned int addr = modulelist[moduleId].baseAddr;
    while (!((McSPIChannelStatusGet(addr, cs) & MCSPI_CH_RXFFE))) {
            McSPIReceiveData(addr, cs);
    }
}
...
void SPIRead_blockPoll(unsigned int moduleId, unsigned char cs, void *prebuf, unsigned int prebuflen,void *buf,unsigned int buflen) {
    unsigned int addr = modulelist[moduleId].baseAddr;
    ASSERT(cs < 3);
    McSPICSAssert(addr, cs);
    McSPIChannelEnable(addr, cs);
    SPIClearBuf(moduleId,cs);
    unsigned int i=0,j=0,k=0;
    while(k<buflen){
        if (i < prebuflen) {
            if(McSPITransmitData(addr, *((unsigned char *)prebuf + i), cs))
                 i++;
        } else if(i < prebuflen+buflen) {
            if(McSPITransmitData(addr, *((unsigned char *)buf + i - prebuflen), cs))
                 i++;
        }else{
            if(McSPITransmitData(addr, 0, cs))
                 i++;
        }

        if (!(McSPIChannelStatusGet(addr, cs) & MCSPI_CH_RXFFE)) {
            unsigned char data = McSPIReceiveData(addr, 0);
            j++;
            if (j>prebuflen) {
                k = j- prebuflen;
                ((unsigned char *)buf)[k-1] = data;
            }
        }
    }
    while(!(McSPIChannelStatusGet(addr, cs) & MCSPI_CH_STAT_EOT));
    McSPICSDeAssert(addr, cs);
    McSPIChannelDisable(addr, cs);
}
...
void SPIWrite_blockPoll(unsigned int moduleId, unsigned char cs, void *prebuf, unsigned int prebuflen, void *buf, unsigned int buflen) {
    ASSERT(cs < 3);
    unsigned int addr = modulelist[moduleId].baseAddr;
    McSPICSAssert(addr, cs);
    McSPIChannelEnable(addr, cs);
    for (int i = 0;i < prebuflen + buflen; ) {
        if (i < prebuflen) {
            if(McSPITransmitData(addr, *((unsigned char *)prebuf + i), cs))
              i++;
        } else {
...
void SPIMasterInit_poll(unsigned int moduleId, unsigned char csChanel, unsigned int spiClk, unsigned char cpha, unsigned char cpol, unsigned char csPolar,unsigned char wordLen) {
   ASSERT(cpha < 2);
   ASSERT(cpol < 2);
   ASSERT((wordLen >= 4) && (wordLen <= 32));
   ASSERT(csChanel <= 3);
